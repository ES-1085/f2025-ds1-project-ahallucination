...
```{r monthly-stacked-area, message = FALSE, warning = FALSE, fig.alt = "Stacked area chart showing monthly hallucination cases from 2023 to 2025 for the top three hallucination types. Fake citations are the most frequent overall and skyrocket in 2025."}
library(tidyverse)
library(lubridate)

hallucination <- c("Fake Citation", "Fabricated Case", "Fake Source/Quote")
...
month_df <- ai_data |>
  ...
  filter(
    year >= 2023, year <= 2025,
    hallucination %in% hallucination
  ) |>
  mutate(
    month = floor_date(Date, unit = "month")
  ) |>
  count(month, hallucination, name = "n") |>
  mutate(
    hallucination = factor(hallucination, levels = c("Fake Citation", "Fabricated Case", "Fake Source/Quote"))
  )
...
ggplot(month_df,
       aes(x = month, y = n, fill = hallucination)) +
  geom_area(alpha = 0.85, color = "white", linewidth = 0.2) +
  scale_fill_viridis_d(option = "D", na.translate = FALSE) +
  scale_x_date(
...
```{r monthly-stacked-hallucinations, message = FALSE, warning = FALSE}
library(tidyverse)
library(lubridate)
