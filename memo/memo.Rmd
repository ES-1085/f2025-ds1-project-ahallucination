...
monetary_penalty_tidy <- ai_data %>%
  mutate(
    ...
    currency = str_to_upper(currency)  # normalize currency codes
  ) %>%
  left_join(rates, by = "currency") %>%     # JOIN EXCHANGE RATES
  mutate(
    monetary_penalty_usd = monetary_penalty * rate_to_usd
  )
```
