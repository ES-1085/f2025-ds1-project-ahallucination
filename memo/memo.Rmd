...
First, we renamed the variables in the dataset.
...
Next, we did some general cleanup for six of our variables by removing extra spaces, capitalizing each word, and converting all empty cells to N/A.
...
Although we did not end up using the monetary penalty variable for our final plots, we cleaned it up by first establishing the currency exchange rates, then converting everything to lowercase and making empty cells NA. Finally, we converted all amounts into USD using the exchange rate table, and replaced all original values with the USD conversion.

```{r monetary-penalty-cleanup}
...
monetary_penalty_clean <- ai_data |>
  mutate(
    monetary_penalty_clean = monetary_penalty |>
      str_replace_all("[$,]", "") |>
      str_squish(),
    monetary_penalty_clean = na_if(str_to_lower(monetary_penalty_clean), "none"),
    monetary_penalty_clean = na_if(monetary_penalty_clean, "n/a"),
    monetary_penalty_clean = na_if(monetary_penalty_clean, "na"),
    monetary_penalty_clean = na_if(monetary_penalty_clean, "N/A"),
    currency = str_extract(
      str_to_lower(monetary_penalty_clean),
      "(usd|cad|ils|eur|gbp|aud)"
    ),
    currency = str_to_upper(currency),
    monetary_amount = readr::parse_number(monetary_penalty_clean),
    currency = if_else(is.na(currency) & !is.na(monetary_amount),
                       "USD", currency)
  ) |>
  left_join(rates, by = "currency") |>
  mutate(
    monetary_penalty_usd = monetary_amount * rate_to_usd,
    currency = if_else(!is.na(monetary_amount), "USD", NA_character_)
  )
...
Finally, we cleaned the professional sanction variable. We also did not use this variable in our final plots, but the cleanup consisted of standardizing all variations of yes/y and no/n to be “Yes” and “No” for readability.

```{r professional-sanction-cleanup}
...
First, we cleaned the AI tool variable. To do this, we got rid of extra spaces and punctuation, converted all text to lowercase, and made all empty cells NA. Then, we made a list of synonyms for all of the most frequently used LLM’s, and finally used the mutate() command to replace all variations of each AI tool spelling with a standardized version.
...
Next, we found and ranked the amount of hallucination cases for each type of LLM each month. Our first chunk of code converts each date to a real date object and extracts the first day of that month. Next, we identified which LLM was used, then counted the number of cases for each tool each month. Finally, we computed the LLM with the largest monthly value and ordered them from largest to smallest.


```{r plot-one-general-cleanup}
...
We aimed to analyze monthly trends in hallucination cases by comparing occurrences across different LLM tools. To ensure consistency, we formatted all dates as year-month-day and grouped cases by month. We filtered for rows with entries in the ‘ai_tool’ column. Using case_when(), we standardized tool names (ChatGPT, MS CoPilot, Gemini) regardless of synonyms or capitalization. We focused analysis on the top three LLMs based on reported cases. For clarity in the legend, we listed LLMs in descending order of total cases by grouping and counting entries. We adjusted the y-axis to fit the maximum number of cases found, using the max_cases variable for customization. To avoid overcrowding the x-axis, we displayed axis breaks every 6 months instead of every month. Finally, we chose the “Dark2” palette from RColorBrewer for accessibility.

```{r line-graph-trend-by-LLM, message = FALSE, warning = FALSE, fig.alt = "Line graph from 2023 to 2025 of number of AI hallucination cases by the 3 most used LLM types represented by three color-coded lines. OpenAI/ChatGPT, MS Copilot, and Gemini are the three large language models plotted. Cases spike in the beginning of 2025 with OpenAI/ChatGPT cases inreasing the most."}
...
ggsave("trendbyLLM.png", width = 6, height = 4, units = "in")
...
First, we standardized the court locations across the dataset. Our code uses the case_when() command to look through each court location and check it against a series of synonyms and alternative spellings/abbreviations used to refer to different states. When a match is found, the text is replaced with our standardized state name. All international courts were changed to “International,” since the purpose of this step was to clean up the variable to use in our US state pie map. 
...
Next, we had to clean the outcome variable. We took the same approach as in the previous step, using the mutate() and case_when() commands to standardize the names of each case outcome into either “Penalized,” “Warning,” “Dismissed,” or “Pending.”
...
Finally, we had to simplify the case outcomes and prepare them for the map plot. We began by creating a new variable that kept only our four standardized outcome categories. We then removed states not included in the map. Next, we counted how many times each outcome occurred in each state and reshaped the data into a wide format with one row per state. Then, we counted each state’s total number of cases and matched the data to geographic center points for plotting. Finally, we assigned color values for each outcome category and prepared a clean U.S. map layout to support the visualization of state-level outcome distributions.

```{r plot-two-general-cleanup}
...
For this plot, our goal was to show how hallucination case outcomes vary by state. We focused on the main four categories (“Warning,” “Dismissed,” “Pending,” “Penalized”) and excluded other outcomes, such as “NA.” We standardized court_location capitalization to align with state names and excluded Hawaii and Puerto Rico due to their smaller mapped areas, which made them difficult to visualize alongside the other mainland states. For each state and outcome, we counted the number of cases, reshaped the data so each outcome was a separate column, and calculated total_cases for pie-chart sizing. To make case comparisons between states more visually intuitive, we sized each state’s pie chart according to its total number of reported cases. We placed the pies at each state’s geographic center, adjusting their radius based on case count for clear visualization. We used the map_data() function along with a “state” data frame for the U.S. basemap and overlaid scatterpie charts, where each slice represented the proportion of each outcome in a state. We defined outcome columns explicitly and used a colorblind-friendly palette (Okabe–Ito) for accessibility.

```{r state-pie-map-cases-by-outcome, message = FALSE, warning = FALSE, fig.alt = "State Pie map of the magnitude of AI hallucination cases and corresponding outcome by state. Relative size of the pie charts correlates to the number of cases in the state. New York and California have the highest number of cases. Slices are color-coded by outcome and show the majority of New York and California's cases resulted in either a warning or penalty."}
...
ggplot() +
  ...
  theme(
    ...
    plot.margin = margin(t = 5, r = 10, b = 5, l = 40),
...
ggsave("CasesbyOutcome.png", width = 6, height = 4, units = "in")
...
First, we standardized the AI tool variable. We used the same method as for the court location and outcome variables cleanup.
...
Finally, we converted all empty cells to NA for the standardized outcome variable, then identified the five most common hallucination types. We calculated the top hallucinations by frequency and filtered the data to include only those categories with valid outcomes. We then counted how often each outcome occurred for each hallucination type and calculated both totals and percentages within each outcome group.
...
We created a bubble plot to compare hallucination types against case outcomes. To order the x-axis meaningfully, we used fct_reorder() to sort outcomes by their total number of cases, ensuring the most common outcomes appear last. Each bubble represents a hallucination type–outcome pair, with its size showing how often that pairing occurs. Filled points with black outlines provided sharper contrast, and bubble sizes were set for distinct visibility. We applied a viridis color scale for accessibility, clearly labeled the axes for readability. Finally, we used a minimal theme and removed minor gridlines to highlight the data patterns.

```{r bubble-map-top-five-hallucinations-by-outcome, fig.alt = "Bubble map of the number of cases for each combination of the four outcomes and top five hallucination types. Color of the bubbles correspond to the type of AI hallucination and relative size of the bubbles represents the number of cases under each intersection of variables. The majority of cases contain fake citations. Fake sources/quotes and fabricated cases have a relatively similar amount of cases across each outcome."}
...
ggplot(hall_outcome,
  ...
  labs(
    title = "Top 5 Hallucination Types by Outcome",
    x = "Outcome",
...
ggsave("HallucinationybyOutcome.png", width = 6, height = 4, units = "in")
